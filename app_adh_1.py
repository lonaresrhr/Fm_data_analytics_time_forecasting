...
if uploaded_file is not None:
	...
	#data1 = st.cache(pd.read_csv)('AdhunikAlloys_Amalgam Steel_Stack2Gas_Nov.csv',parse_dates=True,index_col='Timestamp')
	data1 = st.cache(pd.read_csv)('AdhunikAlloys_Amalgam Steel_Stack2Gas_Nov.csv')
	data1['Datetime'] = pd.to_datetime(data1.Timestamp ,format='%d-%m-%Y %H:%M') 
...
try:
	...
	data3['Datetime'] = pd.to_datetime(data2.Timestamp ,format='%d-%m-%Y %H:%M') 
i=data3
